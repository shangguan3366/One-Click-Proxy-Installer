"生成的 Hysteria2 密码: ${LAST_HY2_PASSWORD}"
...
install_hysteria2_only() {
    ...
    LAST_HY2_PORT=$(ask_port "请输入 Hysteria2 监听端口" "$DEFAULT_HYSTERIA_PORT")
    LAST_HY2_MASQUERADE_CN=$(ask_with_default "请输入 Hysteria2 伪装域名/证书CN" "$DEFAULT_HYSTERIA_MASQUERADE_CN")
...
install_reality_only() {
    ...
    LAST_REALITY_PORT=$(ask_port "请输入 Reality (VLESS) 监听端口" "$DEFAULT_REALITY_PORT")
    LAST_REALITY_SNI=$(ask_with_default "请输入 Reality 目标SNI/握手服务器" "$DEFAULT_REALITY_SNI")
...
uninstall_singbox() {
    if ! confirm_action "你确定要卸载 Sing-box 吗? 此操作将停止并禁用服务，删除可执行文件和相关配置文件目录。是否继续卸载?"; then
        info "卸载已取消。"
        return
    fi
    ...
    if confirm_action "是否删除配置文件目录 ${SINGBOX_CONFIG_DIR} (包含导入信息缓存)?"; then
    ...
    if confirm_action "是否删除 Hysteria2 证书目录 ${HYSTERIA_CERT_DIR}?"; then
    ...
    case "$action" in
        ...
        edit_config)
            if [ -f "$SINGBOX_CONFIG_FILE" ]; then
                ...
                if $SINGBOX_CMD check -c "$SINGBOX_CONFIG_FILE"; then
                    success "配置文件语法正确。"
                    if confirm_action "是否立即重启 sing-box 服务？"; then
                        manage_singbox "restart"
                    fi
                else
                    error "配置文件语法有误，请重新编辑。"
                fi
            else
                error "配置文件不存在: ${SINGBOX_CONFIG_FILE}"
            fi
